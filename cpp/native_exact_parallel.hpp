...
namespace evrptw::native_parallel {
...
native_kernels::ExactBatchOutput run_exact_charging_parallel(
    NativeWorkPool& pool,
    ...
    std::vector<native_kernels::ExactBatchOutput> chunk_outputs(
        chunk_count);
    ...
    pool.parallel_for(chunk_count, [&](const std::size_t chunk) {
        ...
        chunk_workers[chunk] = NativeWorkPool::current_worker_index();
