...
def _verify_committed_runner_identity(tool_identity: dict[str, Any]) -> None:
    ...
    for relative, key in (
        ("tools/run_txnopt_level1_campaign.py", "runner_sha256"),
        ("tools/txnopt_level1_campaign_common.py", "common_sha256"),
    ):
        blob = subprocess.run(
...
def _reviewer_identity() -> dict[str, Any]:
    ...
    for path, key in (
        ("tools/review_txnopt_level1_campaign.py", "reviewer_tool_sha256"),
        ("tools/txnopt_level1_campaign_common.py", "common_tool_sha256"),
    ):
        blob = subprocess.run(
