...
from evrptw.native_execution import (
    ...
    NativeCandidateIncrementalState,
...
class _Evaluator:
    ...
    def candidate_route_batch(
        ...
        *,
        ...
        if self.candidate_transaction_runtime is not None:
            ...
            if self.native_execution_config is not None:
    ...
    def _native_protocol_candidate_route_batch(
        self,
        ...
        *,
        ...
        """Run one explicit Stage 5.2 candidate-round worker transaction."""

        execution = self.native_execution_config
        transaction_runtime = self.candidate_transaction_runtime
        control_runtime = self.candidate_control_runtime
        native_runtime = self.native_runtime
        if (
            execution is None
            or transaction_runtime is None
            or control_runtime is None
            or native_runtime is None
        ):
            raise RuntimeError("explicit native candidate protocol runtime is incomplete")
        if execution.mode != "per_solve_runtime":
            raise RuntimeError("candidate-round dispatch requires per_solve_runtime")
        ...
        incremental_state = self._candidate_incremental_state(
            clean,
            base_sequences=base_sequences,
            native_runtime=native_runtime,
        )
        ...
        if controller is not None and controller.budget is not None:
            controller_remaining = min(
                controller_remaining,
                max(0, controller.budget - controller.started_calls),
            )
        effective_budget = min(controller_remaining, control_runtime.round_remaining)
        route_cache_snapshot = (
        ...
        controller_snapshot = (
            ...
            if controller is None
                ...
                controller.interrupted_calls,
                controller.boundary_recorded,
            )
        )
        ...
        try:
            ...
            native_result = execute_native_candidate_round(
                ...
                NativeCandidateRoundRequest(
                    candidates=clean,
                    cache_hit_flags=cache_flags,
                    proposal_top_k=control_runtime.config.proposal_top_k,
            ...
            self._record_native_incremental_receipts(
                clean,
                base_sequences=base_sequences,
            ...
            if selected != journal_selected:
                raise RuntimeError(
                    "native Candidate Control selection diverged from pure Python projection"
                )
            control_runtime.commit_route_candidate_projection(selection_events)
            ...
            exact_results = dict(
                zip(
                    native_result.exact_candidate_ids,
                    native_result.exact_results,
                    strict=True,
                )
            )
            ...
            for index, resolution in enumerate(native_result.resolutions):
                if resolution == "screening_rejected":
                    ...
                    if screening.native_status(index) != "negative_cache_hit":
                        self.pending_negative_screening_sequences[clean[index]] = reason
                ...
                elif resolution == "cache_hit":
                    resolved[index] = cached_results[index]
                    ...
                    if source < 0 or source >= index or resolved[source] is None:
                        raise RuntimeError("native duplicate result source is invalid")
                    resolved[index] = resolved[source]
                else:
            ...
            transaction_runtime.record_worker_protocol(
                native_result.audit,
                worker_protocol=execution.worker_protocol,
                total_seconds=native_result.timings["total_seconds"],
                queue_wait_seconds=native_result.timings["queue_wait_seconds"],
                completion_order=native_result.completion_order,
            )
            native_runtime.record_screening(
                native_result.timings["screening_seconds"],
                batch_candidates=len(clean),
            )
            if self.measurement_trace is not None:
                self.measurement_trace.record_screening_aggregate(
                    {
                        "event_type": "native_candidate_round",
                        "status": "committed",
                        "timestamp_seconds": self.measurement_trace._offset(),
                        "worker_protocol": execution.worker_protocol,
                        "completion_order": native_result.completion_order,
                        **asdict(native_result.audit),
                    },
                    calls=native_result.audit.input_candidates,
                    passes=native_result.audit.screening_passes,
                    rejections=native_result.audit.screening_rejections,
                    cache_hits=native_result.audit.screening_cache_hits,
                    exact_call_blocked=native_result.audit.screening_exact_call_blocked,
                    reason_counts=native_result.audit.screening_reason_counts,
                )
    ...
    def _candidate_incremental_state(
        self,
        clean: tuple[tuple[str, ...], ...],
        *,
        base_sequences: Sequence[tuple[str, ...]] | None,
        native_runtime: NativeKernelRuntime,
    ) -> NativeCandidateIncrementalState | None:
        """Pack cached base snapshots without crossing the Python/native boundary."""
        ...
        name_to_index = native_runtime.context.name_to_index
        chain_offsets = [0]
        chain_indices: list[int] = []
        edge_offsets = [0]
        edge_values: list[float] = []
        earliest_offsets = [0]
        earliest_values: list[float] = []
        latest_offsets = [0]
        latest_values: list[float] = []
        for base_sequence in base_sequences:
            snapshot = self.propagation_snapshots.get(base_sequence)
            if snapshot is None:
                snapshot = build_route_propagation_snapshot(
                    self.instance,
                    base_sequence,
                    epsilon=epsilon,
                )
                self.propagation_snapshots[base_sequence] = snapshot
            chain_indices.extend(name_to_index[name] for name in snapshot.chain)
            chain_offsets.append(len(chain_indices))
            edge_values.extend(snapshot.edge_distances)
            edge_offsets.append(len(edge_values))
            earliest_values.extend(snapshot.earliest_arrivals)
            earliest_offsets.append(len(earliest_values))
            latest_values.extend(snapshot.latest_departures)
            latest_offsets.append(len(latest_values))
        return NativeCandidateIncrementalState(
            chain_offsets=np.ascontiguousarray(chain_offsets, dtype=np.int64),
            chain_indices=np.ascontiguousarray(chain_indices, dtype=np.int64),
            edge_offsets=np.ascontiguousarray(edge_offsets, dtype=np.int64),
            edge_values=np.ascontiguousarray(edge_values, dtype=np.float64),
            earliest_offsets=np.ascontiguousarray(earliest_offsets, dtype=np.int64),
            earliest_values=np.ascontiguousarray(earliest_values, dtype=np.float64),
            latest_offsets=np.ascontiguousarray(latest_offsets, dtype=np.int64),
            latest_values=np.ascontiguousarray(latest_values, dtype=np.float64),
        )

    def _record_native_incremental_receipts(
        self,
        clean: tuple[tuple[str, ...], ...],
        *,
        base_sequences: Sequence[tuple[str, ...]] | None,
        native_result: NativeCandidateRoundResult,
    ) -> None:
        if base_sequences is None:
            return
        receipts = native_result.incremental_propagation
        if len(receipts) != len(clean):
            raise RuntimeError("native candidate round lost incremental propagation rows")
        reasons = {
        ...
        for base_sequence, candidate, receipt in zip(base_sequences, clean, receipts, strict=True):
            ...
            if status == "incremental":
                self.incremental_propagations += 1
                self.incremental_reused_prefix_edges += receipt.reused_prefix_edges
                self.incremental_reused_suffix_edges += receipt.reused_suffix_edges
            else:
                self.incremental_fallbacks += 1
            if self.measurement_trace is not None:
                self.measurement_trace.record_incremental_propagation(
                    ...
                    candidate_sequence=candidate,
                    status=status,
                    reason=reasons[receipt.reason_code],
                    distance_lower_bound=receipt.distance_lower_bound,
                    min_time_window_slack=receipt.min_time_window_slack,
                    finish_time=receipt.finish_time,
                    reused_prefix_edges=receipt.reused_prefix_edges,
                    reused_suffix_edges=receipt.reused_suffix_edges,
                    recomputed_forward_edges=receipt.recomputed_forward_edges,
                    recomputed_backward_edges=receipt.recomputed_backward_edges,
                )
    ...
    def _native_candidate_route_batch(
        ...
        *,
        ...
        incremental_rows = self._candidate_incremental_rows(
            clean,
            base_sequences=base_sequences,
            native_runtime=native_runtime,
        )
...
def _solve_alns(
    ...
    *,
    ...
    evaluator = _Evaluator(
        ...
        native_runtime=native_runtime,
        native_execution_config=native_execution_config,
    )
...
def _full_native_operator_statistics(
    ...
    *,
    ...
    native_operator_totals: npt.NDArray[np.int64] | None = None,
    ...
    initial_report = validate_routes(
        instance,
        [list(result.route) for result in initial_exact],
    )
    if not initial_report.feasible:
        raise RuntimeError("full native operator replay warm start failed validation")
    initial_objective = SolutionObjective.from_report(instance, initial_report)
    current_by_lane = {
        "legacy": initial_objective,
        "quality_shadow": initial_objective,
        "constraint_lane": initial_objective,
    }
    best_objective = initial_objective
    grouped: dict[tuple[int, str], list[dict[str, object]]] = {}
    for event in semantic_events:
        operator = str(event["operator"])
        if operator not in statistics:
            raise RuntimeError("full native semantic event used an unknown operator")
        grouped.setdefault((cast(int, event["iteration"]), operator), []).append(event)
    ...
    for (iteration, _), events in grouped.items():
        ...
        values["candidate_proposals"] = cast(int, values["candidate_proposals"]) + sum(
        ...
        accepted_events = [event for event in events if bool(event["accepted"])]
        ...
        candidate_objective = SolutionObjective(
            vehicle_count=objective_key[0],
            total_distance=objective_key[1],
            total_charging_time=objective_key[2],
            charging_count=objective_key[3],
        )
        event_track = str(event.get("track", ""))
        lane = (
            "constraint_lane"
            if event_track == "constraint_lane"
            or operator
            in {
                "station_pressure",
        ...
        if comparison == "better":
            values["improved"] = cast(int, values["improved"]) + 1
        if is_global_best:
            values["best"] = cast(int, values["best"]) + 1
    ...
    native_arrays = (
        ...
        native_operator_calls,
        native_operator_rewards,
    )
    if any(value is not None for value in native_arrays):
        if any(value is None for value in native_arrays):
            raise RuntimeError("full native operator evidence is incomplete")
        assert native_operator_totals is not None
        assert native_operator_activity is not None
        assert native_operator_calls is not None
        assert native_operator_rewards is not None
        if (
            native_operator_totals.shape != (len(FULL_NATIVE_OPERATOR_NAMES), 8)
            or native_operator_activity.shape != (len(FULL_NATIVE_OPERATOR_NAMES), 8)
            or native_operator_calls.shape != (len(FULL_NATIVE_OPERATOR_NAMES),)
            or native_operator_rewards.shape != (len(FULL_NATIVE_OPERATOR_NAMES),)
        ):
            raise RuntimeError("full native operator evidence has an invalid shape")
        for index, name in enumerate(FULL_NATIVE_OPERATOR_NAMES):
...
def _full_native_maximum_stagnation(
    instance: Instance,
    initial_customer_sequences: tuple[tuple[str, ...], ...],
    semantic_events: tuple[dict[str, object], ...],
    ...
    for sequence in initial_customer_sequences:
        ...
        if not exact.feasible:
            raise RuntimeError("full native initial incumbent replay is infeasible")
        initial += SolutionObjective.from_route(
            instance,
            exact.route,
...
def _solve_full_native_alns(
    ...
    *,
    ...
    operator_statistics = _full_native_operator_statistics(
        ...
        native_operator_activity=(
            native_result.semantic_stream.operator_activity
            if isinstance(native_result.semantic_stream, NativeThreeLaneSemanticStream)
            else None
        ),
        native_operator_calls=(
            native_result.semantic_stream.operator_calls
            if isinstance(native_result.semantic_stream, NativeThreeLaneSemanticStream)
            else None
        ),
        native_operator_rewards=(
            native_result.semantic_stream.operator_rewards
            if isinstance(native_result.semantic_stream, NativeThreeLaneSemanticStream)
            else None
        ),
    ...
    native_stage04_control = (
        native_result.semantic_stream.stage04_control
        if isinstance(native_result.semantic_stream, NativeThreeLaneSemanticStream)
        else None
    )
    ...
    if measurement_trace is not None and measurement_trace.runtime_semantic_enabled:
        ...
        for raw_event in native_result.exact_journal_events:
            ...
            if raw_event.get("event_type") == "candidate_route_result":
                next_exact_evaluation_id += 1
        ...
        def append_exact_context(
            ...
            *,
            ...
            while (
                ...
                consumed_screening += 1
            transaction_screening = pending_screening[:consumed_screening]
            matched_screening = 0
            for sequence in sequences:
                ...
                if matching_index is None:
                    raise RuntimeError(
                        "full native exact work lost its ordered screening prefix: "
                        f"context={context!r}, transaction_id={transaction_id}, "
                        f"route_key={route_key!r}"
                    )
            ...
            pending_producer_events = exact_journal_by_context.get(context, [])
            producer_events: list[dict[str, object]] = []
            producer_result_count = 0
            while pending_producer_events:
                producer_event = pending_producer_events.pop(0)
                producer_events.append(producer_event)
                if producer_event.get("event_type") == "candidate_route_result":
                    producer_result_count += 1
                if (
                    producer_event.get("event_type") == "parallel_batch"
            ...
            if (
                ...
                raise RuntimeError(
                    ...
                    f"context={context!r}, requested={requested}"
                )
            if pending_producer_events:
                exact_journal_by_context[context] = pending_producer_events
            else:
                exact_journal_by_context.pop(context, None)
            producer_results = [
                event
                for event in producer_events
                if event.get("event_type") == "candidate_route_result"
            ]
            pending_rich_results = exact_results_by_context.get(context, [])
            rich_results = pending_rich_results[:requested]
            del pending_rich_results[:requested]
            if pending_rich_results:
            ...
            if pending_evaluation_ids:
                exact_evaluation_ids_by_context[context] = pending_evaluation_ids
            else:
                ...
                producer_bindings.append(
                    {
                        ...
                        "route_key": measurement_trace.register_route(
                            cast(list[list[str]], event["customer_sequences"])[0]
                        ),
                        "lane": context[0],
                        "iteration": context[1],
                        "operator": context[2],
            ...
            if producer_bindings != rich_bindings:
                raise RuntimeError(
        ...
        for initialization_context in tuple(screening_by_context):
        ...
        for iteration in range(native_result.counters["iterations"]):
            ...
            for lane in ("legacy", "quality_shadow", "constraint_lane"):
                ...
                for screening_context in tuple(screening_by_context):
                    if screening_context[:2] == (lane, iteration):
                        append_screening_context(screening_context)
                if lane == "quality_shadow" and iteration_completed:
            ...
            stale_screening_contexts = tuple(
                context
                for context in screening_by_context
                if context[1] is None or context[1] <= iteration
            )
            if stale_screening_contexts:
                raise RuntimeError(
    ...
    result = ALNSResult(
        ...
        neighborhood_events=semantic_events,
        failure_reason="",
...
def solve_alns(
    ...
    *,
    ...
    if (
        native_kernel_config is not None
        and candidate_control_enabled
        and native_execution_config is None
    ):
        raise ValueError(
    ...
    if (
        ...
        try:
            ...
            return _attach_native_execution_statistics(
                result,
                native_execution_config,
            )
        finally:
            if candidate_control_runtime is not None:
    ...
    trace.finish(result)
    return _attach_native_execution_statistics(
        replace(result, measurement_trace=trace),
        native_execution_config,
    )


def _attach_native_execution_statistics(
    result: ALNSResult,
    config: Stage052NativeExecutionConfig | None,
) -> ALNSResult:
    if config is None:
        return result
    transaction_fallbacks = result.candidate_transaction_statistics.get(
    ...
    statistics["candidate_transactions"] = result.candidate_transaction_statistics.get(
    ...
    return replace(result, native_execution_statistics=statistics)
