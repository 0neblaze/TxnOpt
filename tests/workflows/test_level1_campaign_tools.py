...
from txnopt_evidence.level1_campaign_reviewer import (
    _aggregate,
    _audit_t4_waste_events,
    _gate_results,
    _semantic_exact_work_started,
)
from txnopt_evidence.level1_campaign_runner import (
...
def test_reviewer_requires_a_clean_committed_identity(
    ...
    from txnopt_evidence.level1_campaign_reviewer import _reviewer_identity

    monkeypatch.setattr(
        "txnopt_evidence.level1_campaign_reviewer.require_clean_repository",
...
def test_review_rejects_an_execution_receipt_without_clean_tool_identity(
    ...
    from txnopt_evidence.level1_campaign_reviewer import _validate_execution_receipt
...
def test_reviewer_does_not_import_the_raw_producer_module() -> None:
    reviewer = Path("src/txnopt_evidence/level1_campaign_reviewer.py").read_text(
        encoding="utf-8"
    )
    assert "from txnopt_evidence.level1_campaign_runner import" not in reviewer


def test_linux_resource_floor_measurements_are_positive() -> None:
    assert _physical_core_count() > 0
    from txnopt_evidence.level1_campaign_runner import _memory_gib
