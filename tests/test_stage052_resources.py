...
def test_formal_resource_recalibration_v3_binds_cgroup_memory_and_rerun02(
    tmp_path: Path,
) -> None:
    contract = ProducerResourceContract(
        selected_workers=6,
    ...
    assert evidence.predecessor_run_label == "stage05.2_benchmark_rerun02"
    assert evidence.predecessor_batch_id == "batch0008"
...
def test_formal_resource_recalibration_v3_rejects_shared_user_manager(
    ...
    with pytest.raises(RuntimeError, match="isolated cgroup v2 memory evidence"):
