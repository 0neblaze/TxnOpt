...
PRE_REFACTOR_TAG = "txnopt-pre-refactor-v1"


def _protected_input_bytes(relative_path: str) -> bytes:
    current = ROOT / relative_path
    if current.is_file():
        return current.read_bytes()
    return subprocess.run(
        ["git", "-C", str(ROOT), "show", f"{PRE_REFACTOR_TAG}:{relative_path}"],
        check=True,
        capture_output=True,
    ).stdout
...
def test_formal_receipt_binds_checked_in_models_and_generated_translation() -> None:
    ...
    for relative_path, expected in receipt["input_sha256"].items():
        assert hashlib.sha256(_protected_input_bytes(relative_path)).hexdigest() == expected
