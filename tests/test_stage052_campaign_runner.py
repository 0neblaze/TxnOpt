...
def test_campaign_successor_revision_allows_only_g_governance_paths(
    ...
    allowed_governance = repository / "src" / "evrptw" / "storage_governance.py"
    allowed_governance.write_text("v1\n", encoding="utf-8")
    subprocess.run(("git", "-C", str(repository), "add", "."), check=True)
    subprocess.run(
    ...
    allowed_governance.write_text("v2\n", encoding="utf-8")
...
def test_campaign_root_preflight_accepts_bound_ntfs_usb_archive(
    ...
    monkeypatch.setattr(campaign_runner.os, "name", "nt")
    locator = StorageRootLocator(
        {
            "wsl_staging": StorageRoot(
                "wsl_staging",
