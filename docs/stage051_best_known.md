...
The v6 reviewer compares every canonical BKS and compatibility field,
including source/compilation references, DOI, charging fields, and
`model_compatible`; it requires the canonical single-level
`results/<run_label>/` layout, a clean revision, and the published Stage 4 v6
...
by the v6 reviewer. It also rejects partial raw bundles before any CSV replay.
